...
<template>
    <div class="space-y-4">
        ...
        >
            ...
            <div
                v-if="loading"
                class="py-16 text-xs text-center text-slate-400"
            >
            ...
            <div v-else>
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        ...
                        <tbody
                            class="text-xs divide-y divide-slate-100 text-slate-600"
                        >
                            <tr
                                v-for="row in paginated.data"
                                :key="row.ids[0]"
                            ...
                            >
                                ...
                                <td class="p-4">
                                    ...
                                    >
                                        <i
                                            class="mr-1 fa-solid fa-clock text-slate-400"
                                        ></i
                                        >{{ row.time_label }}
                                        <span
                                            v-if="row.hours > 1"
                                            class="text-[10px] bg-blue-50 text-blue-600 border border-blue-200 px-1.5 py-0.5 rounded-full font-medium ml-1"
                                        >
                                            {{ row.hours }} ชม.
                                        </span>
                                    </div>
                            ...
                            <tr v-if="!paginated.data.length">
                                <td
                                    colspan="5"
                                    class="text-xs text-center py-14 text-slate-400"
                                >
                                    <i
                                        class="block mb-2 text-2xl fa-solid fa-inbox"
                                    ></i>
                                    ไม่มีรายการ
                                </td>
                            </tr>
                ...
                >
                    <p class="text-[11px] text-slate-400">
                        แสดง {{ paginated.from }}–{{ paginated.to }} จาก
                        {{ paginated.total }} รายการ
                    </p>
                    <div class="flex items-center gap-1">
                        <button
                            @click="fetch(paginated.current_page - 1)"
                        ...
                        <template v-for="p in pageNumbers" :key="p">
                            ...
                            <button
                                v-else
                                @click="fetch(p as number)"
                                :class="
                                    p === paginated.current_page
                                        ? 'bg-blue-900 text-white border-blue-900'
                                        : 'text-slate-600 border-slate-200 hover:bg-slate-100'
                                "
                                class="text-xs font-medium border rounded-lg w-7 h-7"
                            >
                                {{ p }}
                            </button>
                        </template>
                        ...
                        >
                            <i
                                class="fa-solid fa-chevron-right text-[10px]"
                            ></i>
                        </button>
                    </div>
                </div>
